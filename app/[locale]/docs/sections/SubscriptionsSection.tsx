...
curl -X POST https://api.cipherpay.app/api/subscriptions/{id}/cancel \\
  ...
  -d '{"at_period_end": true}'`} />
      ...
      <Paragraph>
        ...
        an <Code>active</Code> boolean that accounts for whether the current billing period is still valid.
      </Paragraph>
      <CodeBlock lang="bash" code={`curl https://api.cipherpay.app/api/subscriptions/{id}/status \\
  -H "Authorization: Bearer cpay_sk_YOUR_KEY"`} />
      <Paragraph>
        Response:
      </Paragraph>
      <CodeBlock lang="json" code={`{
  "subscription_id": "csub_abc123",
  "active": true,
  "status": "active",
  ...
  "cancel_at_period_end": false
      ...
      <Paragraph>
        ...
        subscription status is <Code>active</Code> <Strong>and</Strong> the current billing period hasn&apos;t ended.
      </Paragraph>

      <SectionDivider />

      <SectionTitle>Managing subscriptions in the dashboard</SectionTitle>
      <Paragraph>
        The merchant dashboard includes a <Strong>Subscriptions</Strong> tab under the Commerce section of the sidebar.
        From here you can:
      </Paragraph>
      ...
      <SectionTitle>Quick-start checklist</SectionTitle>
      <div style={{ fontSize: 11, color: 'var(--cp-text-dim)', lineHeight: 2.4, marginBottom: 16 }}>
        1. Create a product with a <Strong>recurring price</Strong> (dashboard or API)<br />
        2. Register a <Strong>webhook URL</Strong> in your dashboard settings<br />
