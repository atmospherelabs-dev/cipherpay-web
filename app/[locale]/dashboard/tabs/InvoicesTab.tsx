...
export const InvoicesTab = memo(function InvoicesTab({
  ...
  const createPaymentLink = async () => {
    ...
    if (!amount || amount <= 0) {
      showToast(t('toastValidAmount'), true);
      return;
    }
    ...
    try {
      ...
      reloadInvoices();
      showToast(t('toastLinkCreated'));
    } catch (err) {
      showToast(err instanceof Error ? err.message : t('toastFailedCreateLink'), true);
    }
    setPayLinkCreating(false);
  };

  const cancelInvoice = async (id: string) => {
    try { await api.cancelInvoice(id); reloadInvoices(); showToast(t('toastCancelled')); }
    catch { showToast(t('toastFailedCancel'), true); }
  };
      ...
      <div className="panel">
        <div className="panel-header">
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            {invoices.length > 0 && (
              <button onClick={exportInvoicesCSV} className="btn btn-small">{t('exportCsv')}</button>
            )}
        ...
        {showPayLinkForm && (
          <div className="panel-body" style={{ borderBottom: '1px solid var(--cp-border)' }}>
            <div style={{ fontSize: 10, color: 'var(--cp-text-dim)', letterSpacing: 1, marginBottom: 8 }}>{t('createPaymentLink')}</div>
            ...
            <div className="form-group">
              ...
              <input type="text" value={payLinkDesc} onChange={(e) => setPayLinkDesc(e.target.value)} placeholder={t('descPlaceholder')} className="input" />
            </div>
                ...
                {isExpanded && (
                  <div onClick={(e) => e.stopPropagation()} style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--cp-border)' }}>
                    <div className="stat-row">
                      <span style={{ color: 'var(--cp-text-muted)' }}>{t('reference')}</span>
                    ...
                    <div className="stat-row">
                      <span style={{ color: 'var(--cp-text-muted)' }}>{t('rateAtCreation')}</span>
                    ...
                    <div className="section-title" style={{ marginTop: 12 }}>{t('timeline')}</div>
                    <div className="stat-row">
                      <span style={{ color: 'var(--cp-text-muted)' }}>{t('created')}</span>
                      <span style={{ fontSize: 10 }}>{new Date(inv.created_at).toLocaleString()}</span>
                    </div>
                    {inv.detected_at && (
                      <div className="stat-row">
                        <span style={{ color: 'var(--cp-text-muted)' }}>{t('detected')}</span>
                    ...
                    {inv.confirmed_at && (
                      <div className="stat-row">
                        <span style={{ color: 'var(--cp-green)' }}>{t('confirmed')}</span>
                    ...
                    {!inv.confirmed_at && !inv.refunded_at && (
                      <div className="stat-row">
                        <span style={{ color: 'var(--cp-text-muted)' }}>{t('expires')}</span>
                    ...
                    <div className="stat-row" style={{ marginTop: 12 }}>
                      <CopyButton text={`${checkoutOrigin}/pay/${inv.id}`} label={t('paymentLinkLabel')} />
